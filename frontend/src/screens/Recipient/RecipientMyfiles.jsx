...
const RecipientMyFiles = () => {
  const [data,setData] = useState([]);

  useEffect(()=>{
    const fetchData = async()=>{
        await apiHelper.get('/api/recipient/sharedfiles').then((res)=>setData(res.data));
    ...
    fetchData();
      ...
      return (
        <div className='w-full h-full py-4 flex flex-col gap-4'>
          <h1 className='py-4 text-c_blue font-bold text-xl'>Shared Files</h1>
    ...
    <tr key={index} className="text-black">
              ...
              <td className="py-3 px-4">{items.original.filename}</td>
            
              <td className="py-3 px-4 ">{items.sender.name}</td>
...
export default RecipientMyFiles